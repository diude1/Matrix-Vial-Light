...
|---|---|---|---|---|
| `amk` | **Matrix 官方固件**（Faukwaa 实测）、AMK 系固件 | 轴灯 + 配件灯 + 指示灯 + 点阵屏，各自灯效 / 亮度 / 速度 / 颜色 | △ 矩阵轴灯（视固件） | ✓ 轴灯/配件灯**独立设灯效** |
...
> **5 条配件灯条、共 18 颗灯**，每条还能各选 10 档灯效（0–9）。
>
...
> **AMK 后端的分区页没有逐灯坐标**。概览里轴灯按矩阵 `rows × cols` 平铺，
> 配件灯每条占一列排在右侧；逐键页会在矩阵通道可用时允许轴灯逐键上色，
> 配件灯仅显示并引导到分区页（实现见 `VialDevice.amk_leds()`）。
...
### 逐键（VialRGB；AMK 视矩阵通道而定）
- 按每颗 LED 的 `(x, y)` 坐标自动排布成键盘图：细边是轴灯，亮蓝边是配件灯。
- 工具：**画笔**（拖动上色）/ **取色** / **填充**（整盘同色）。
- 图案：水平渐变 / 垂直渐变 / 彩虹 / 波浪 / 全部同色 / 清空。
- AMK 设备的轴灯在矩阵通道可用时支持逐键上色；AMK 配件灯在此页只读，
  要改灯条请到「分区」页。VialRGB 的配件灯仍可按 LED 子集编辑。
- **本地动画**：仅对 VialRGB 启用，由电脑以约 30fps 持续推送 fastset；
  AMK 会把该开关置灰。
...
- **AMK 的逐键能力取决于矩阵通道**：分区页只能整组控制轴灯，但支持矩阵自定义档
  的固件可以在逐键页编辑轴灯；配件灯没有矩阵坐标，只在逐键页显示，必须到分区页
  按灯条编辑。
- **AMK 轴灯灯效 1–45 没有权威名称**：固件不提供「读取灯效列表」也不提供
  「翻译名称」的命令，所以除少数已确认档位外只显示「模式 N」。
- **AMK 分区页的轴灯是整组写色**（`0x83`）；若矩阵通道支持自定义档，
  逐键页另走 `SET_RGB_MATRIX_LED(48)` 编辑轴灯。配件灯必须逐灯写
...
- 逐键配色不会由通用灯光保存命令自动持久化；本地方案可以保存已知的逐键颜色，
  应用方案时会重新推送。VialRGB 的 Direct 模式本身仍是内存态。
